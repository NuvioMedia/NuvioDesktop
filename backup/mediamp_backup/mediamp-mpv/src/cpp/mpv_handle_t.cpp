...
// ---- Dynamic libmpv loading (ported from NuvioDesktop) ----

template <typename T>
T MpvDynApi::load_symbol(const char *name) {
    using FuncType = T;
    FARPROC sym = nullptr;
#ifdef _WIN32
    sym = GetProcAddress((HMODULE)library, name);
#else
    sym = (FARPROC)dlsym(library, name);
#endif
    if (!sym) {
        load_failure = std::string("libmpv is missing export ") + name;
#ifdef _WIN32
...
void MpvDynApi::ensure_loaded() {
    if (library) return;
    do_load();
    if (!library) {
        throw std::runtime_error(load_failure.empty() ? "Unable to load libmpv." : load_failure);
    }
}

void MpvDynApi::do_load() {
#ifdef _WIN32
    std::vector<std::wstring> candidates;
    wchar_t envPath[32768] = {};
    DWORD envLength = GetEnvironmentVariableW(L"NUVIO_LIBMPV_PATH", envPath, (DWORD)(sizeof(envPath)/sizeof(envPath[0])));
    if (envLength > 0 && envLength < sizeof(envPath)/sizeof(envPath[0])) {
    ...
    if (GetModuleFileNameW(nullptr, modulePath, MAX_PATH)) {
        ...
        if (pos != std::wstring::npos) {
            ...
            candidates.insert(candidates.begin(), dir + L"\\libmpv-2.dll");
        }
    }
    ...
    if (!library) {
        load_failure = "Unable to load libmpv-2.dll. Install mpv or set NUVIO_LIBMPV_PATH.";
        return;
    }
#elif defined(__APPLE__)
    std::vector<std::string> candidates;
    const char *envPath = getenv("NUVIO_LIBMPV_PATH");
    if (envPath && envPath[0]) candidates.push_back(envPath);
    ...
    candidates.push_back("/opt/local/lib/libmpv.dylib");

    Dl_info info;
    if (dladdr((void*)do_load, &info) && info.dli_fname) {
        std::string path(info.dli_fname);
        auto pos = path.find_last_of("/");
        if (pos != std::string::npos) {
            path = path.substr(0, pos + 1) + "libmpv.dylib";
    ...
    if (!library) {
        load_failure = "Unable to load libmpv.dylib. Install mpv via Homebrew or set NUVIO_LIBMPV_PATH.";
        return;
    }
#else
    // Linux
    ...
    candidates.push_back("/usr/lib/x86_64-linux-gnu/libmpv.so");
    candidates.push_back("/usr/lib/libmpv.so");
    ...
    if (!library) {
        load_failure = "Unable to load libmpv.so. Install mpv via your package manager or set NUVIO_LIBMPV_PATH.";
        return;
    }
#endif

    create_fn = load_symbol<mpv_create_fn>("mpv_create");
    if (!library) return;
    initialize_fn = load_symbol<mpv_initialize_fn>("mpv_initialize");
    if (!library) return;
    terminate_destroy_fn = load_symbol<mpv_terminate_destroy_fn>("mpv_terminate_destroy");
    if (!library) return;
    set_option_fn = load_symbol<mpv_set_option_fn>("mpv_set_option");
    if (!library) return;
    set_option_string_fn = load_symbol<mpv_set_option_string_fn>("mpv_set_option_string");
    if (!library) return;
    set_property_fn = load_symbol<mpv_set_property_fn>("mpv_set_property");
    if (!library) return;
    set_property_string_fn = load_symbol<mpv_set_property_string_fn>("mpv_set_property_string");
    if (!library) return;
    get_property_fn = load_symbol<mpv_get_property_fn>("mpv_get_property");
    if (!library) return;
    command_fn = load_symbol<mpv_command_fn>("mpv_command");
    if (!library) return;
    error_string_fn = load_symbol<mpv_error_string_fn>("mpv_error_string");
    if (!library) return;
    free_fn = load_symbol<mpv_free_fn>("mpv_free");
    if (!library) return;
    wait_event_fn = load_symbol<mpv_wait_event_fn>("mpv_wait_event");
    if (!library) return;
    wakeup_fn = load_symbol<mpv_wakeup_fn>("mpv_wakeup");
    if (!library) return;
    request_log_messages_fn = load_symbol<mpv_request_log_messages_fn>("mpv_request_log_messages");
    if (!library) return;
    observe_property_fn = load_symbol<mpv_observe_property_fn>("mpv_observe_property");
    if (!library) return;
    unobserve_property_fn = load_symbol<mpv_unobserve_property_fn>("mpv_unobserve_property");
    if (!library) return;
    render_context_create_fn = load_symbol<mpv_render_context_create_fn>("mpv_render_context_create");
    if (!library) return;
    render_context_free_fn = load_symbol<mpv_render_context_free_fn>("mpv_render_context_free");
    if (!library) return;
    render_context_render_fn = load_symbol<mpv_render_context_render_fn>("mpv_render_context_render");
    if (!library) return;
    render_context_set_update_callback_fn = load_symbol<mpv_render_context_set_update_callback_fn>("mpv_render_context_set_update_callback");
}

MpvDynApi &mpv_api() {
    static MpvDynApi api;
    api.ensure_loaded();
    return api;
}
...
void mpv_handle_t::create(JNIEnv *env, jobject app_context) {
	...
	handle_ = api.create_fn();
	api.request_log_messages_fn(handle_, "terminal-default");
}
...
if (api.initialize_fn(handle_) < 0) {
...
FP;
CHECK_HANDLE()
return mpv_api().command_fn(handle_, args) >= 0;
}

bool mpv_handle_t::set_option(const char *key, const char *value) {
FP;
CHECK_HANDLE()
return mpv_api().set_option_string_fn(handle_, key, value);
}

template bool mpv_handle_t::get_property(const char *name, mpv_format format, int64_t *out_result);
template bool mpv_handle_t::get_property(const char *name, mpv_format format, double *out_result);
template bool mpv_handle_t::get_property(const char *name, mpv_format format, int *out_result);
template bool mpv_handle_t::get_property(const char *name, mpv_format format, char **out_result);

template<typename T>
bool mpv_handle_t::get_property(const char *name, mpv_format format, T *out_result) {
FP;
CHECK_HANDLE()
return mpv_api().get_property_fn(handle_, name, format, out_result) >= 0;
}

template bool mpv_handle_t::set_property(const char *name, mpv_format format, int64_t *in_value);
template bool mpv_handle_t::set_property(const char *name, mpv_format format, double *in_value);
template bool mpv_handle_t::set_property(const char *name, mpv_format format, int *in_value);
template bool mpv_handle_t::set_property(const char *name, mpv_format format, const char **in_value);

template<typename T>
bool mpv_handle_t::set_property(const char *name, mpv_format format, T *in_value) {
FP;
CHECK_HANDLE()
return mpv_api().set_property_fn(handle_, name, format, in_value) >= 0;
}

bool mpv_handle_t::observe_property(const char *property, mpv_format format, uint64_t reply_data) {
FP;
CHECK_HANDLE()
return mpv_api().observe_property_fn(handle_, reply_data, property, format) >= 0;
}

bool mpv_handle_t::unobserve_property(uint64_t reply_data) {
FP;
CHECK_HANDLE()
return mpv_api().unobserve_property_fn(handle_, reply_data) >= 0;
}
...
surface_ = ref;
surface_attached_ = mpv_api().set_option_fn(handle_, "wid", MPV_FORMAT_INT64, &wid) >= 0;
...
int64_t wid = 0;
bool result = mpv_api().set_option_fn(handle_, "wid", MPV_FORMAT_INT64, (void*) &wid);
...
bool mpv_handle_t::attach_window_surface(int64_t wid) {
	FP;
	CHECK_HANDLE();
	return mpv_api().set_option_fn(handle_, "wid", MPV_FORMAT_INT64, &wid) >= 0;
}
...
if (mpv_api().render_context_create_fn(&render_context_, handle_, params) < 0) {
render_context_ = nullptr;
wglMakeCurrent(old_dc, old_ctx);
return false;
}

mpv_api().render_context_set_update_callback_fn(render_context_, mpv_render_update_callback, static_cast<void*>(handle_));
	...
	if (device_ptr != 0) {
		...
		if (current_egl_ctx != EGL_NO_CONTEXT) {
			...
			if (load_gl_functions()) {
				...
				if (mpv_api().render_context_create_fn(&render_ctx, handle_, params) >= 0) {
					mpv_api().render_context_set_update_callback_fn(render_ctx, mpv_render_update_callback, static_cast<void*>(handle_));
...
return false;
	...
	mpv_api().render_context_free_fn(render_context_);
...
// 无论是否有新帧，都调用 render（mpv 文档建议）
	int render_result = mpv_api().render_context_render_fn(render_context_, params);
	if (render_result < 0) {
...
mpv_api().terminate_destroy_fn(handle_);
