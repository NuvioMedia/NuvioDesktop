...
val linuxCflags = providers.gradleProperty("linuxCflags").getOrElse("")

val buildLinuxPlayerBridge = tasks.register<Exec>("buildLinuxPlayerBridge") {
    enabled = isLinuxHost
    val srcFile = linuxPlayerBridgeSource.asFile
    val outFile = linuxPlayerBridgeOutput.get().asFile
    val javaHome = linuxPlayerBridgeJavaHome
    val cc = linuxCc
    val extraCflags = linuxCflags

    inputs.file(linuxPlayerBridgeSource)
    inputs.property("cc", linuxCc)
    inputs.property("cflags", linuxCflags)
    outputs.file(linuxPlayerBridgeOutput)
    doFirst {
        val javaIncludes = "-I${javaHome}/include -I${javaHome}/include/linux"
        val mpvPkg = "mpv"
        val cflagsOut = try {
            ProcessBuilder("pkg-config", "--cflags", mpvPkg)
                .redirectErrorStream(true)
                .start()
                .inputStream.bufferedReader().readText()
        ...
        val libs = try {
            ProcessBuilder("pkg-config", "--libs", mpvPkg)
                .redirectErrorStream(true)
                .start()
                .inputStream.bufferedReader().readText()
                .trim().takeIf { it.isNotBlank() } ?: ""
        } catch (_: Exception) { "" }
        val extraFlags = extraCflags.split(" ").filter { it.isNotBlank() }
        commandLine(
            cc, "-shared", "-fPIC",
            "-Wl,-rpath,'\$ORIGIN'",
            ...
            *cflagsOut.split(" ").filter { it.isNotBlank() }.toTypedArray(),
