...
impl MpvPlayer {
    ...
    fn dispatch_end_file(&self, event: &mpv_event) {
        if event.data.is_null() {
            return;
        }
        let end = unsafe { &*(event.data as *const mpv_event_end_file) };
        let reason = end.reason;
        let error_code = end.error;

        let error_str = if error_code != 0 {
            unsafe {
                let ptr = (self.fns.mpv_error_string)(error_code);
                if !ptr.is_null() {
        ...
        eprintln!("[mpv-rust] MPV_EVENT_END_FILE reason={} error='{}' playlist_entry_id={}", reason, error_str, end.playlist_entry_id);
        ...
        if let (Some(listener), Some(jvm)) = (listener_guard.as_ref(), jvm_guard.as_ref()) {
            if let Ok(mut env) = jvm.attach_current_thread() {
                if let Ok(j_error_str) = env.new_string(&error_str) {
                    let _ = env.call_method(
                        listener.as_obj(),
                        "onEndFile",
                        "(ILjava/lang/String;)V",
                        &[
                            jni::objects::JValue::Int(reason),
                            jni::objects::JValue::Object(&j_error_str),
                        ],
                    );
                }
            }
    ...
    pub fn command(&self, args: &[&str]) -> bool {
        let ctx = self.ctx_ptr();
        if ctx.is_null() {
            eprintln!("[mpv-rust-debug] command SKIP: ctx is null");
            return false;
        }
        ...
        let result = unsafe { (self.fns.mpv_command)(ctx, raw_ptrs.as_ptr()) == 0 };
        eprintln!("[mpv-rust-debug] command args={:?} result={}", args, result);
        result
    }
